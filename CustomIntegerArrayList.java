...
public class CustomIntegerArrayList {
	...
	public void remove(int num, int element) {
		...
		Iterator itr = this.arr.iterator();
		if(num <= 0){
			return;
		}
		int count = 0;
	...
	public ArrayList<Integer> splice(int index, int num) {
		...
		int lengthOfArray = this.arr.size();
		if(index < 0 || num <= 0 || index >= lengthOfArray){
			return empty;
		}
		int finalIndex = Math.min(lengthOfArray,index+num);
		int numberOfElementsToBeRemoved = finalIndex - index;
	...
	public ArrayList<Integer> splice(int index, int num, int[] otherArray) {
		...
		int finalIndex = Math.min(lengthOfArray-1,index+num);
		int numberOfElements = finalIndex - index;
		for(int i=0;i<numberOfElements;i++){
			this.arr.remove(index);
		}
		int otherArrayLength = otherArray.length;
		for(int i=index,j=0;j<otherArrayLength;i++,j++){
			this.arr.add(i,otherArray[j]);
		}
		return this.arr;
	}
	...
	public static void main(String args[]) {
		...
		System.out.println();
		System.out.println();
		System.out.println(arr3.getArrayList());
		arr3.remove(2, 10);
		//arr3.remove(2);
		System.out.println(arr3.getArrayList());
	}
